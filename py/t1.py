...
class Spider(Spider):
    ...
    def init(self, extend):
        ...
        self.fileUrl = extendDict['file_url'].strip('/') if 'file_url' in extendDict else ''
    ...
    def homeContent(self, filter):
        self.log("执行homeContent函数》》》》》》》》》》》》》》》》》》》")
        try:
            embyInfos = self.getAccessToken()
        except Exception as e:
            return {'msg': f"{f'获取Emby服务器信息出错{e}'}"}
        ...
        r = requests.get(url, params=params, headers=header, timeout=120,
    ...
    def detailContent(self, did):
        ...
        vod = {
            ...
            "vod_pic": f'{self.baseUrl}/emby/Items/{did[0]}/Images/Primary?maxWidth=400&tag={videoInfos["ImageTags"]["Primary"]}&quality=90' if 'Primary' in
                                                                                                                                                videoInfos[
                                                                                                                                                    'ImageTags'] else '',
    ...
    def playerContent(self, flag, pid, vipFlags):
        ...
        r = requests.post(url, params=params, data=data, headers=header, timeout=120,
                          proxies={"http": self.proxy, "https": self.proxy})
        url = self.baseUrl + r.json()['MediaSources'][0]['DirectStreamUrl']
        if self.fileUrl:
            url = self.fileUrl + r.json()['MediaSources'][0]['DirectStreamUrl']
    ...
    def getAccessToken(self):
        ...
        r = requests.post(f"{self.baseUrl}/emby/Users/AuthenticateByName",
                          params={"Username": self.username, "Password": self.password, "Pw": self.password,
                                  "X-Emby-Client": "Emby Theater", "X-Emby-Device-Name": "DESKTOP-C25AFR6",
                                  "X-Emby-Device-Id": str(uuid4()), "X-Emby-Client-Version": "3.0.20-3.0"},
                          headers=header, timeout=120, proxies={"http": self.proxy, "https": self.proxy})
        embyInfos = r.json()
    ...
    header = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) EmbyTheater/3.0.20-3.0 Chrome/100.0.4896.160 Electron/18.3.15 Safari/537.36"}
