...
class Spider(Spider):
    ...
    def categoryContent(self, cid, page, filter, ext):
        ...
        if page != '1':
            url = f"{self.home_url}/{cid}/{_year}/{_class}/{_area}/page/{page}"  # 使用版本B的分頁格式
        ...
        try:
            ...
            for group in collection_info.get('videosGroup', []):
                ...
                if is_movie:
                    ...
                    play_url.append(f"{vod_name}${video['purl']}")  # 使用版本A的直接 purl
                else:
                    episodes = []
                    for video in group['videos']:
                        ep_name = f"第{video['eporder']}集"
                        ep_url = video['purl']  # 使用版本A的直接 purl
